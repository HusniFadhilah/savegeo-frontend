...
export default function ImageryToolsPanel({ scene, aoi, assetKey, onNasaLayer, onStory, mapMode = "flat", onMapMode }: {
  ...
  return <div className="mt-3">
    <button type="button" className="btn btn-success w-100 text-start d-flex justify-content-between align-items-center" aria-expanded={toolsOpen} onClick={() => setToolsOpen((value) => !value)}>
      <span><i className="bi bi-tools me-2" />{t("imagery.tools.title")}</span><i className={`bi bi-chevron-${toolsOpen ? "up" : "down"}`} />
    </button>
    {!toolsOpen ? null : <div className="accordion mt-2" id="imageryToolsAccordion">
