...
export function KarhutlaDetailPage() {
  ...
  useEffect(() => { if (!event) return; const from = query.from || event.monitoring_from; const to = query.to || event.last_data_at?.slice(0, 10) || new Date().toISOString().slice(0, 10); let cancelled = false; setLoading(true); fetchWildfireHotspots(event.slug, { from, to, sensor: query.sensor === "all" ? undefined : query.sensor, confidence: query.confidence.join(",") }).then((hotspotResponse) => { if (cancelled) return; setSummary(hotspotResponse.summary); setFeatures(hotspotResponse.features); setTimeline(hotspotResponse.timeline ?? []); }).catch(() => { if (cancelled) return; setSummary(summaryFromEvent(event)); setFeatures([]); setTimeline([]); }).finally(() => { if (!cancelled) setLoading(false); }); return () => { cancelled = true; }; }, [event, query.from, query.to, query.sensor, query.confidence.join(",")]);
  ...
  const timelineData = timeline.length ? timeline : [{ date: event.monitoring_from, count: activeSummary.total_hotspots }];
