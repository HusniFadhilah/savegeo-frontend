...
(async()=>{
 ...
 await page.screenshot({path:'test-results/globe/current.png', timeout:5000}).catch(()=>{}); await page.locator('.globe-settings > summary').click();
 ...
 // Chromium does not emit a second watchPosition callback reliably when its
 // emulated coordinates are changed. The watcher itself is verified here;
 // callback/update semantics are covered by useUserGeolocation.test.ts.
 await page.waitForTimeout(500);
 assert(await page.getByRole('button',{name:'Stop following',exact:true}).isVisible());
 ...
 await page.screenshot({path:'test-results/globe/desktop.png', timeout:5000}).catch(()=>{});
 await page.setViewportSize({width:390,height:844});await page.waitForTimeout(1000);
 await page.screenshot({path:'test-results/globe/mobile.png', timeout:5000}).catch(()=>{});
 ...
 assert.equal(await page.evaluate(()=>window.locationState.getState().latitude),-6.234567);
